/*
pack is a fork of go-bindata tailored to creating flotilla assets

pack converts any file into a managable asset.AssetFS. Useful for
...
ready for deployment, just re-invoke `pack` without the `-debug` flag.
...
passed to `pack`. This includes the path. In most cases, this is not
desireable, as it puts potentially sensitive information in your code base.
For this purpose, the tool supplies another command line flag `-stripPrefix`.
This accepts a portion of a path name, which should be stripped off from
the map keys and function names.
For example, running without the `-prefix` flag, we get:

	$ pack /path/to/templates/
	_bindata["/path/to/templates/foo.html"] = path_to_templates_foo_html

Running with the `-prefix` flag, we get:

	$ pack -stripPrefix "/path/to/" /path/to/templates/
